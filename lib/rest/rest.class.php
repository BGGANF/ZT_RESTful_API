...
class rest
{
    public function curl($url, $type = 'GET', $option = array(), $header = array())
    {
        if($type == 'GET' && $option)
        {
            $query = '&';
        ...
        $logInfo = $this->log(round($total, 2), $type, $url, $option, $result);
    ...
    public function log($time, $type, $url, $option, $result)
    {
        /* Set the error info. */
