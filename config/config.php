...
$config->apiVersion  = 'v1';           // The version number, don't change.

$config->referer   = 'http://nerd.ag.listcloud.cn/';

$config->ag = new stdClass();
$config->ag->host = 'http://nerd.ag.listcloud.cn';
$config->ag->appID = 'ICETEST0-A1BA-4D38-B1A5-91A2EFE29118';
$config->ag->token = '0E5049589E198E98BDC6';
$config->encrypt = 'commonICE';

$config->mail = new stdClass();
$config->mail->smtp = 'smtp.exmail.qq.com';
$config->mail->from = 'support@inerd.com.cn';
$config->mail->password = 'Inerd789456';
$config->mail->template = '<a href="%s">点击这个进行认证</a>';
$config->mail->subject = '验证您的身份信息 诺德';

/* 引用自定义的配置。 Include the custom config file. */
$myConfig = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'my.php';
if(file_exists($myConfig)) include $myConfig;
$myConfig = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'front.php';
if(file_exists($myConfig)) include $myConfig;
$myConfig = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'error.php';
if(file_exists($myConfig)) include $myConfig;
$myConfig = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'dbtable.php';
if(file_exists($myConfig)) include $myConfig;
