   ...
   /**
    ...
    public function setRouteByPathInfo()
    {
        if(!empty($this->URI))
        {
            ...
            if(strpos($this->URI, $apiVersion) === 0) $this->URI = substr($this->URI, strlen($apiVersion) + 1);
            /*
    ...
    public function loadModule()
    {
        ...
        $result = call_user_func_array(array($module, $methodName), $this->params);
        if(is_array($result)){
...
//            foreach (fixer::input('get')->get() as $key => $value){
//                $params[$key] = $value;
//            }
            $_GET = $params;    //for RESTful
        }
    ...
    public function mergeParams($defaultParams, $passedParams)
    {
        ...
        foreach($passedParams as $param => $value)
        {
            ...
            if(preg_match('/[^\x{4e00}-\x{9fa5}a-zA-Z0-9=_\-]/', trim($value))) die('Bad Request!');
        }
        ...
        foreach($defaultParams as $key => $defaultValue)
        {
            ...
            {
                $defaultParams[$key] = strip_tags(urldecode($passedParams[$key]));
            }
            else
            {
